...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark"  >
  ...
  <div class="collapse navbar-collapse" id="navbarTogglerDemo02">
    <ul class="navbar-nav mr-auto mt-2 mt-lg-0">
      ...
      <a class="nav-link"  href="http://localhost/5edmty/projet/ordinateur_portable.php?#"><span class="sr-only">Ordinateur Portable</span></a>
      </li>
      <li class="nav-item">
      <a class="nav-link" id="ss" href="ordinateur_de_bureau">Ordinateur De Bureau</a>
      </li>
    </ul>
  </div>
  <?php
...
<?php
 ...
 include("connexion2.php") ; 
 $res_notes = $cnx->query("SELECT * FROM pc_de_bureau WHERE nom LIKE '%{$vsearch}%'"); 
 // Extraire (fetch) toutes les lignes (enregistrement, rows)
 $les_notes = $res_notes->fetchAll(); // Ceci est un tableau de tableaux associatifs
 // Calculer le nombre d'étudiants
 $nbr_etudiants = count($les_notes);
...
echo "<tr><td>" . $item[0]."</td><td>" ."<img src='$item[4]'  width=200px height=200px>"."</td><td>" . $item[1]."</td><td>".$item[2]."</td><td>".$item[3]."</td><td>"."<a href='acheter3.php?pp1=$item[0]'><input type='submit' value='Ajouter au Panier' class='btn btn-success'></a>"."</td></tr>";
}
echo "</table>";
}
?>
